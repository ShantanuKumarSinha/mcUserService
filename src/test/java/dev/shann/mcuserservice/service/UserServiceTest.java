...
class UserServiceTest {
  ...
  void shouldCreateNewUser() {
    ...
    when(userRepository.save(getUserEntity())).thenReturn(getUserEntityWithId());
  ...
  void shouldNotBeAbleToCreateNewUser() {
    when(userRepository.findByEmail(getUser().getEmail())).thenReturn(Optional.empty());
    when(modelMapper.map(getUser(), UserEntity.class)).thenReturn(getUserEntity());
    when(userRepository.save(getUserEntity())).thenReturn(null);
    when(modelMapper.map(null, User.class)).thenReturn(null);
    var user = userService.createUser(createUserDTO());
    assertThat(user).isNull();
  }

  @Test
  void shouldThrowExistingUserException() {
    when(userRepository.findByEmail(getUser().getEmail()))
        .thenReturn(Optional.ofNullable(getUserEntity()));
    assertThrows(ExistingUserException.class, () -> userService.createUser(createUserDTO()));
  }
  ...
  void shouldNotBeAbleToAuthenticate() {
    when(userRepository.findByEmail(anyString()))
        .thenReturn(Optional.ofNullable(any(UserEntity.class)));
    var authenticateUserDTO = authenticateUserDTO();
    var exception =
        assertThrows(
            EmailNotFoundException.class, () -> userService.authenticate(authenticateUserDTO));
